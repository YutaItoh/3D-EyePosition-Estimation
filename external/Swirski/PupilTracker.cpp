/**
 @brief 
 This code is taken and modified from the following external open-source libraries:
...
///		if(minResponse>-200) return false;
	}
    // Paradoxically, a good Haar fit won't catch the entire pupil, so expand it a bit
    haarRadius = (int)(haarRadius * SQRT_2 * 1.8);/// 1.3, *1.5* /// 1.8 for DELOCK
	...
	//std::cout<<"Log in Iris: "<<__LINE__<<std::endl;
...
////	std::vector<cv::Point2f> edgePoints;
	...
	SECTION("Ellipse fitting", log)
	{
		...
		if (edgePoints.size() >= n) // Minimum points for ellipse
		{
			...
			out.A = conicBestFit.A;
			out.B = conicBestFit.B;
			out.C = conicBestFit.C;
			out.D = conicBestFit.D;
			out.E = conicBestFit.E;
			out.F = conicBestFit.F;
		}
