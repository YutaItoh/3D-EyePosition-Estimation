...
inline void getROI(const cv::Mat& src, cv::Mat& dst, const cv::Rect& roi, int borderType = cv::BORDER_REPLICATE)
{
...
inline float histKmeans(const cv::Mat_<float>& hist, int bin_min, int bin_max, int K, float init_centres[], cv::Mat_<uchar>& labels, cv::TermCriteria termCriteria)
{
...
inline cv::RotatedRect fitEllipse(const cv::Moments& m)
{
	cv::RotatedRect ret;
...
inline cv::Vec2f majorAxis(const cv::RotatedRect& ellipse)
{
